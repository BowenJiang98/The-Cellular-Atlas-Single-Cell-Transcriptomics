...
Idents(sce)<- "celltype"
sce <-subset(x = sce, idents = "PTM cells")
setwd("E:/Bowen-files/out/v5_addT90_new/PTM cells/26_3_11")
...
pdf("PTM_cells_3group.pdf",width = 21,height = 7)
plot3
dev.off()

pdf("PTM_cells_group.pdf",width = 7,height = 7)
plot4
dev.off()

pdf("PTM_cells_umap.pdf",width = 7,height = 7)
plot1
dev.off()
save.image("PTM_cell.Rdata")
...
# 第一步：选择用于构建细胞轨迹的基因集；
# 选择Top800差异基因作为排序基因；
ordering_genes <- rownames(clustering_DEG_genes)[order(clustering_DEG_genes$qval)][1:1500]
...
ggsave(Pseudotime_clusters,filename = "PTM_ProPseudotime1500.pdf", width = 7, height = 7,units = "in",dpi = 300)
#按“State”分组；
State_clusters <-plot_cell_trajectory(CDS, color_by = "State")
ggsave(State_clusters,filename = "PTM_State.pdf", width = 7, height = 7,units = "in",dpi = 300)

#按seurat分群结果分组
clusters <-plot_cell_trajectory(CDS, color_by = "seurat_clusters")
ggsave(clusters,filename = "PTM_clusters.pdf", width = 7, height = 7,units = "in",dpi = 300)
#按细胞类型分组
#plot_cell_trajectory(CDS, color_by = "celltype")
#按照ori_ident分类
clusters <-plot_cell_trajectory(CDS, color_by = "group")
ggsave(clusters,filename = "PTM_ORig.ident.pdf", width = 7, height = 7,units = "in",dpi = 300)

save(CDS, file = "CDS_PTM_pseudotime.Rda")
