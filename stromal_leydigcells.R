...
Idents(sce)<- "celltype"
sce <-subset(x = sce, idents = c("Leydig cells","Stromal cells"))
setwd("E:/Bowen-files/out/v5_addT90_new/stromal_leydigcell/26_3_11")
...
Epi_sce =FindClusters(Epi_sce,resolution = 0.02)
table(Epi_sce@meta.data$seurat_clusters) 
Epi_sce <-RunUMAP(Epi_sce, reduction = "harmony", dims =1:20)##reduction="harmony
#EPI_umap图_未注释
Epi_sce$group
...
pdf("Leydig_cells_3group.pdf",width = 21,height = 7)
plot3
dev.off()

pdf("Leydig_stromal_cells_group.pdf",width = 7,height = 7)
plot4
dev.off()

pdf("Leydig_stromal_cells_umap.pdf",width = 7,height = 7)
plot1
dev.off()
save.image("Leydig_stromal_stromal_cell.Rdata")
...
setwd("E:/Bowen-files/out/v5_addT90_new/Leydigcell/26_3_11")
sig_gene_names2 <- c('CST9L','RPL9','VDAC2','RPS8','PGAM1','PABPC1','FOXO3','HMGN1','HSP70.2','RPS2','RPS7','RPL7A')
